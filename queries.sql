...
tab3 AS (     -- В подзапросе tab3 мы находим количество уникальных покупателей
    SELECT DISTINCT -- в разрезе каждого месяца.
        selling_month,
        COUNT(customer_name)
            OVER (PARTITION BY selling_month) as customer_count
    FROM tab2
),
...
WITH tab1 AS (
    SELECT      
        ...
        CONCAT(e.first_name, ' ', e.last_name) AS seller,
        p.price,
        ROW_NUMBER() -- присваиваем номера в партиции клиент, продавец
            OVER (PARTITION BY CONCAT(c.first_name, ' ', c.last_name),  
                CONCAT(e.first_name, ' ', e.last_name) 
                    ORDER BY sale_date) AS flag_1, -- сортировка по дате ASC
        ROW_NUMBER() -- присваиваем номер каждой записи с клиентом для выбора первого
